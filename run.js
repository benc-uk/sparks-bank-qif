...
if(process.argv[2]) {
  ...
  console.log("### No input PDF specified, will default to statement.pdf");
}
...
.then(pdf => {
  ...
  // Horrific regex
  let pdfRegex = /\d\d\s\w\w\w\s\d\d\s\d\d\s\w\w\w\s\d\d.*?£[\d,]+\.\d\d(\sCR)?/mgi;
  ...
  for(let transLine of pdfMatches) {
    ...
    try {
      ...
      if(!transMatches || transMatches.length <= 0) {
        ...
        console.log(`### Bad transaction line was\n ${transLine}\n<END OF LINE>`);
        process.exit(1);
      }
      ...
      console.log(`# ${parseDate(transMatches[2])}\tamount=${amount}\tpayee=${transMatches[3]}`);
    } catch(e) {
